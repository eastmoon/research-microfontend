...
:action
    ...
    docker build -t vue.dev:%PROJECT_NAME% -f Dockerfile.vue .
    cd %CLI_DIRECTORY%
    ...
    @rem start project
    docker rm -f mff-%PROJECT_NAME%-dev-vue
    docker run -ti --rm ^
        ...
        -w /repo ^
        --name mff-%PROJECT_NAME%-dev-vue ^
        vue.dev:%PROJECT_NAME% bash
    goto end
...
:short
    echo Startup vue develop tools
    goto end
